...
let rec sub (v:exp) (x:string) (e:exp) : exp =
  ...
  | ELet (x1, e1, e2) when x <> x1          -> ELet (x1, s e1, s e2)
  | EFunc (x1, e1) when x <> x1             -> EFunc (x1, s e1)
  | EFix (f, x1, e1) when x <> x1 && x <> f -> EFix (f, x1, s e1)
  | EVar x1 when x = x1                     -> v
  | _ as e                                  -> e
...
let rec interpret (e:exp) : exp =
  ...
  and interpretApp (e1:exp) (e2:exp) : exp =
    ...
    match f with
    | EFunc (x, e3)    -> interpret (sub v2 x e3)
    | EFix (f', x, e3) -> interpret (sub f f' (sub v2 x e3))
...
let rec step (e:exp) : exp =
  ...
  | _ as e           -> e
   ...
   and stepLet (x:string) (e1:exp) (e2:exp) : exp =
     if is_value e1 then sub e1 x e2 else ELet (x, step e1, e2)
   and stepApp (e1:exp) (e2:exp) : exp =
     if is_value e1 && is_value e2 then
       match e1 with
       | EFunc (x, e3)   -> sub e2 x e3
       | EFix (f, x, e3) -> sub e1 f (sub e2 x e3)
       | _ -> error (sprintf "Function expected, got %s" (string_of_exp e1))
